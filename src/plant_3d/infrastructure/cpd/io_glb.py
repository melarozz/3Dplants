...
def load_glb(path):
    ...
    for name, geom in scene.geometry.items():
        if not isinstance(geom, trimesh.Trimesh):
            continue
        logger.debug("Mesh '%s': %d vertices", name, len(geom.vertices))
    ...
    logger.info("Loaded %d vertices from '%s' (%d meshes)", len(points), path.name, len(vertices_list))
    return points
...
def export_glb(points, path, *, color = (100, 200, 100, 255)):
...
def export_merged_glb(source_points, target_points, path):
    ...
    src_colors = np.tile([
        ...
        200,
        120,
        255], (len(source_points), 1)).astype(np.uint8)
    tgt_colors = np.tile([
        100,
        149,
        237,
        255], (len(target_points), 1)).astype(np.uint8)
    src_cloud = trimesh.PointCloud(vertices = source_points, colors = src_colors)
    tgt_cloud = trimesh.PointCloud(vertices = target_points, colors = tgt_colors)
    scene = trimesh.Scene([
        src_cloud,
        tgt_cloud])
    scene.export(str(path), file_type = 'glb')
