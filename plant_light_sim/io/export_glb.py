...
def tone_map_relative(values: np.ndarray) -> np.ndarray:
    v = values.copy()

    p1, p99 = np.percentile(v, [1, 99])
    v = np.clip((v - p1) / (p99 - p1 + 1e-8), 0, 1)
...
def tone_map_absolute(values: np.ndarray, max_ppfd: float = 1000.0) -> np.ndarray:
    """
    Absolute mapping:
    1000 µmol/m²/s = max (white/yellow)
    """
    v = values / max_ppfd
    return np.clip(v, 0, 1)


def save_colored_glb(
    vertices,
    faces,
    light_vals,
    output_path,
    ...
    if add_lamp and light_pos is not None:
        ...
        lamp.visual.vertex_colors = np.tile(
            np.array([255, 230, 0, 255], dtype=np.uint8),
            (len(lamp.vertices), 1),
        )
